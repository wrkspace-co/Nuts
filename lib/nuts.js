...
Nuts.prototype.onDownload = function(req, res, next) {
    ...
    // if they used /download/darwin-<arch>, map it to osx_<arch>
    if (!filename && platform && platform.startsWith('darwin-')) {
        var arch = platform.split('-')[1];   // “arm64” or “x64” or “universal”
        platform = platforms.OSX + '_' + arch;
    }
    ...
    if (!filename) {
        ...
        if (!platform) {
            // default mac download — pick the universal build
            if (req.useragent.isMac) platform = platforms.OSX_UNIVERSAL;
...
Nuts.prototype.onUpdateWin = function(req, res, next) {
    ...
    var fullUrl = getFullUrl(req);
    var platform = req.params.platform || 'win32';
    var channel = req.params.channel || 'stable';
    var tag = req.params.version;
